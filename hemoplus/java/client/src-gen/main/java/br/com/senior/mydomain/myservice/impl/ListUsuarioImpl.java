...
public class ListUsuarioImpl {
	...
	public ListUsuarioImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(Usuario.PageRequest input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				userId.getTenant(), //
				MyServiceConstants.DOMAIN, //
				MyServiceConstants.SERVICE, //
				MyServiceConstants.Commands.LIST_USUARIO, //
	...
	*/
	public Usuario.PagedResults listUsuario(Usuario.PageRequest input, long timeout) {
		...
		Usuario.PagedResults output = DtoJsonConverter.toDTO(resultMessage.getContent(), Usuario.PagedResults.class);
		if (output == null) {
	...
	*/
	...
	public CompletableFuture<Usuario.PagedResults> listUsuarioRequest(Usuario.PageRequest input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, Usuario.PagedResults.class);
	}
	...
	* The 'list' request primitive for the Usuario entity.
	*/
	public CompletableFuture<Usuario.PagedResults> listUsuarioRequest(Usuario.PageRequest input, long timeout, TimeUnit unit) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, Usuario.PagedResults.class, TimeUnit.MILLISECONDS.convert(timeout, unit));
	}
