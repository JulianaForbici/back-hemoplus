 ...
 * 
 */
public class MarcarNaoCompareceuPorPeriodoImpl {
	...
	public MarcarNaoCompareceuPorPeriodoImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(MarcarNaoCompareceuPorPeriodoInput input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				userId.getTenant(), //
				MyServiceConstants.DOMAIN, //
				MyServiceConstants.SERVICE, //
				MyServiceConstants.Commands.MARCAR_NAO_COMPARECEU_POR_PERIODO, //
	...
	*/
	public MarcarNaoCompareceuPorPeriodoOutput marcarNaoCompareceuPorPeriodo(MarcarNaoCompareceuPorPeriodoInput input, long timeout) {
		...
		MarcarNaoCompareceuPorPeriodoOutput output = DtoJsonConverter.toDTO(resultMessage.getContent(), MarcarNaoCompareceuPorPeriodoOutput.class);
		if (output == null) {
	...
	*/
	...
	public CompletableFuture<MarcarNaoCompareceuPorPeriodoOutput> marcarNaoCompareceuPorPeriodoRequest(MarcarNaoCompareceuPorPeriodoInput input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, MarcarNaoCompareceuPorPeriodoOutput.class);
	}
	...
	* 
	*/
	public CompletableFuture<MarcarNaoCompareceuPorPeriodoOutput> marcarNaoCompareceuPorPeriodoRequest(MarcarNaoCompareceuPorPeriodoInput input, long timeout, TimeUnit unit) {
		MyServiceValidator.validate(input);
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, MarcarNaoCompareceuPorPeriodoOutput.class, TimeUnit.MILLISECONDS.convert(timeout, unit));
	}
