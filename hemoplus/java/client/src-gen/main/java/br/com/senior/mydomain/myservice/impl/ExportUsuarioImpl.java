...
import br.com.senior.mydomain.myservice.ExportUsuarioInput;
import br.com.senior.mydomain.myservice.ExportUsuarioOutput;
 ...
 * 
 */
public class ExportUsuarioImpl {
	...
	public ExportUsuarioImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(ExportUsuarioInput input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				userId.getTenant(), //
				MyServiceConstants.DOMAIN, //
				MyServiceConstants.SERVICE, //
				MyServiceConstants.Commands.EXPORT_USUARIO, //
	...
	*/
	public ExportUsuarioOutput exportUsuario(ExportUsuarioInput input, long timeout) {
		...
		ExportUsuarioOutput output = DtoJsonConverter.toDTO(resultMessage.getContent(), ExportUsuarioOutput.class);
		if (output == null) {
	...
	*/
	...
	public CompletableFuture<ExportUsuarioOutput> exportUsuarioRequest(ExportUsuarioInput input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, ExportUsuarioOutput.class);
	}
	...
	* 
	*/
	public CompletableFuture<ExportUsuarioOutput> exportUsuarioRequest(ExportUsuarioInput input, long timeout, TimeUnit unit) {
		MyServiceValidator.validate(input);
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, ExportUsuarioOutput.class, TimeUnit.MILLISECONDS.convert(timeout, unit));
	}
