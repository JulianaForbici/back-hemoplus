 ...
 * 
 */
public class RetrieveEstoqueImpl {
	...
	public RetrieveEstoqueImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(Estoque.GetRequest input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				userId.getTenant(), //
				MyServiceConstants.DOMAIN, //
				MyServiceConstants.SERVICE, //
				MyServiceConstants.Commands.RETRIEVE_ESTOQUE, //
	...
	*/
	public Estoque retrieveEstoque(Estoque.GetRequest input, long timeout) {
		...
		Estoque output = DtoJsonConverter.toDTO(resultMessage.getContent(), Estoque.class);
		if (output == null) {
	...
	*/
	...
	public CompletableFuture<Estoque> retrieveEstoqueRequest(Estoque.GetRequest input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, Estoque.class);
	}
	...
	* The 'retrieve' request primitive for the Estoque entity.
	*/
	public CompletableFuture<Estoque> retrieveEstoqueRequest(Estoque.GetRequest input, long timeout, TimeUnit unit) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, Estoque.class, TimeUnit.MILLISECONDS.convert(timeout, unit));
	}
