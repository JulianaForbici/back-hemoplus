...
public class ListEstoqueImpl {
	...
	public ListEstoqueImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(Estoque.PageRequest input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				userId.getTenant(), //
				MyServiceConstants.DOMAIN, //
				MyServiceConstants.SERVICE, //
				MyServiceConstants.Commands.LIST_ESTOQUE, //
	...
	*/
	public Estoque.PagedResults listEstoque(Estoque.PageRequest input, long timeout) {
		...
		Estoque.PagedResults output = DtoJsonConverter.toDTO(resultMessage.getContent(), Estoque.PagedResults.class);
		if (output == null) {
	...
	*/
	...
	public CompletableFuture<Estoque.PagedResults> listEstoqueRequest(Estoque.PageRequest input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, Estoque.PagedResults.class);
	}
	...
	* The 'list' request primitive for the Estoque entity.
	*/
	public CompletableFuture<Estoque.PagedResults> listEstoqueRequest(Estoque.PageRequest input, long timeout, TimeUnit unit) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, Estoque.PagedResults.class, TimeUnit.MILLISECONDS.convert(timeout, unit));
	}
