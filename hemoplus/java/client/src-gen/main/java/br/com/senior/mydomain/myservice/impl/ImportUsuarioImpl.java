...
import br.com.senior.mydomain.myservice.ImportUsuarioInput;
import br.com.senior.mydomain.myservice.ImportUsuarioOutput;
 ...
 * 
 */
public class ImportUsuarioImpl {
	...
	public ImportUsuarioImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(ImportUsuarioInput input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				userId.getTenant(), //
				MyServiceConstants.DOMAIN, //
				MyServiceConstants.SERVICE, //
				MyServiceConstants.Commands.IMPORT_USUARIO, //
	...
	*/
	public ImportUsuarioOutput importUsuario(ImportUsuarioInput input, long timeout) {
		...
		ImportUsuarioOutput output = DtoJsonConverter.toDTO(resultMessage.getContent(), ImportUsuarioOutput.class);
		if (output == null) {
	...
	*/
	...
	public CompletableFuture<ImportUsuarioOutput> importUsuarioRequest(ImportUsuarioInput input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, ImportUsuarioOutput.class);
	}
	...
	* 
	*/
	public CompletableFuture<ImportUsuarioOutput> importUsuarioRequest(ImportUsuarioInput input, long timeout, TimeUnit unit) {
		MyServiceValidator.validate(input);
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, ImportUsuarioOutput.class, TimeUnit.MILLISECONDS.convert(timeout, unit));
	}
