 ...
 * 
 */
public class BuscarDashboardSerieAgendamentosPorDiaImpl {
	...
	public BuscarDashboardSerieAgendamentosPorDiaImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(BuscarDashboardSerieAgendamentosPorDiaInput input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				userId.getTenant(), //
				MyServiceConstants.DOMAIN, //
				MyServiceConstants.SERVICE, //
				MyServiceConstants.Commands.BUSCAR_DASHBOARD_SERIE_AGENDAMENTOS_POR_DIA, //
	...
	*/
	public BuscarDashboardSerieAgendamentosPorDiaOutput buscarDashboardSerieAgendamentosPorDia(BuscarDashboardSerieAgendamentosPorDiaInput input, long timeout) {
		...
		BuscarDashboardSerieAgendamentosPorDiaOutput output = DtoJsonConverter.toDTO(resultMessage.getContent(), BuscarDashboardSerieAgendamentosPorDiaOutput.class);
		if (output == null) {
	...
	*/
	...
	public CompletableFuture<BuscarDashboardSerieAgendamentosPorDiaOutput> buscarDashboardSerieAgendamentosPorDiaRequest(BuscarDashboardSerieAgendamentosPorDiaInput input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, BuscarDashboardSerieAgendamentosPorDiaOutput.class);
	}
	...
	* 
	*/
	public CompletableFuture<BuscarDashboardSerieAgendamentosPorDiaOutput> buscarDashboardSerieAgendamentosPorDiaRequest(BuscarDashboardSerieAgendamentosPorDiaInput input, long timeout, TimeUnit unit) {
		MyServiceValidator.validate(input);
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, BuscarDashboardSerieAgendamentosPorDiaOutput.class, TimeUnit.MILLISECONDS.convert(timeout, unit));
	}
