...
import br.com.senior.mydomain.myservice.ConfirmarAgendamentoInput;
import br.com.senior.mydomain.myservice.ConfirmarAgendamentoOutput;
 ...
 * 
 */
public class ConfirmarAgendamentoImpl {
	...
	public ConfirmarAgendamentoImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(ConfirmarAgendamentoInput input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				userId.getTenant(), //
				MyServiceConstants.DOMAIN, //
				MyServiceConstants.SERVICE, //
				MyServiceConstants.Commands.CONFIRMAR_AGENDAMENTO, //
	...
	*/
	public ConfirmarAgendamentoOutput confirmarAgendamento(ConfirmarAgendamentoInput input, long timeout) {
		...
		ConfirmarAgendamentoOutput output = DtoJsonConverter.toDTO(resultMessage.getContent(), ConfirmarAgendamentoOutput.class);
		if (output == null) {
	...
	*/
	...
	public CompletableFuture<ConfirmarAgendamentoOutput> confirmarAgendamentoRequest(ConfirmarAgendamentoInput input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, ConfirmarAgendamentoOutput.class);
	}
	...
	* 
	*/
	public CompletableFuture<ConfirmarAgendamentoOutput> confirmarAgendamentoRequest(ConfirmarAgendamentoInput input, long timeout, TimeUnit unit) {
		MyServiceValidator.validate(input);
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, ConfirmarAgendamentoOutput.class, TimeUnit.MILLISECONDS.convert(timeout, unit));
	}
