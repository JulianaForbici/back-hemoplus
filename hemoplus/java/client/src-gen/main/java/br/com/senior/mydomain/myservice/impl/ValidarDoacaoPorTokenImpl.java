 ...
 * 
 */
public class ValidarDoacaoPorTokenImpl {
	...
	public ValidarDoacaoPorTokenImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(ValidarDoacaoPorTokenInput input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				userId.getTenant(), //
				MyServiceConstants.DOMAIN, //
				MyServiceConstants.SERVICE, //
				MyServiceConstants.Commands.VALIDAR_DOACAO_POR_TOKEN, //
	...
	*/
	public ValidarDoacaoPorTokenOutput validarDoacaoPorToken(ValidarDoacaoPorTokenInput input, long timeout) {
		...
		ValidarDoacaoPorTokenOutput output = DtoJsonConverter.toDTO(resultMessage.getContent(), ValidarDoacaoPorTokenOutput.class);
		if (output == null) {
	...
	*/
	...
	public CompletableFuture<ValidarDoacaoPorTokenOutput> validarDoacaoPorTokenRequest(ValidarDoacaoPorTokenInput input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, ValidarDoacaoPorTokenOutput.class);
	}
	...
	* validacao de doacao por token
	*/
	public CompletableFuture<ValidarDoacaoPorTokenOutput> validarDoacaoPorTokenRequest(ValidarDoacaoPorTokenInput input, long timeout, TimeUnit unit) {
		MyServiceValidator.validate(input);
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, ValidarDoacaoPorTokenOutput.class, TimeUnit.MILLISECONDS.convert(timeout, unit));
	}
