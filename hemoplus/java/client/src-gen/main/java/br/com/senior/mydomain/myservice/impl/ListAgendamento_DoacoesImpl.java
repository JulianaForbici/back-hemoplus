 ...
 * 
 */
public class ListAgendamento_DoacoesImpl {
	...
	public ListAgendamento_DoacoesImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(Doacao.PageRequest input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				userId.getTenant(), //
				MyServiceConstants.DOMAIN, //
				MyServiceConstants.SERVICE, //
				MyServiceConstants.Commands.LIST_AGENDAMENTO_DOACOES, //
	...
	*/
	public Doacao.PagedResults listAgendamento_Doacoes(Agendamento.Id parentId, Doacao.PageRequest input, long timeout) {
		...
		Doacao.PagedResults output = DtoJsonConverter.toDTO(resultMessage.getContent(), Doacao.PagedResults.class);
		if (output == null) {
	...
	*/
	...
	public CompletableFuture<Doacao.PagedResults> listAgendamento_DoacoesRequest(Agendamento.Id parentId, Doacao.PageRequest input) {
		...
		addMessageHeaders(message);
		message.addHeader("entity_parent_id", parentId.id);
		return serviceRunner.request(message, Doacao.PagedResults.class);
	}
	...
	* The 'list' request primitive for the doacoes attribute of agendamento entity.\nThis primitive requires the header 'entity_parent_id' with id of parent agendamento entity.
	*/
	public CompletableFuture<Doacao.PagedResults> listAgendamento_DoacoesRequest(Agendamento.Id parentId, Doacao.PageRequest input, long timeout, TimeUnit unit) {
		...
		addMessageHeaders(message);
		message.addHeader("entity_parent_id", parentId.id);
		return serviceRunner.request(message, Doacao.PagedResults.class, TimeUnit.MILLISECONDS.convert(timeout, unit));
	}
