...
import br.com.senior.mydomain.myservice.ImportEstoqueInput;
import br.com.senior.mydomain.myservice.ImportEstoqueOutput;
 ...
 * 
 */
public class ImportEstoqueImpl {
	...
	public ImportEstoqueImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(ImportEstoqueInput input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				userId.getTenant(), //
				MyServiceConstants.DOMAIN, //
				MyServiceConstants.SERVICE, //
				MyServiceConstants.Commands.IMPORT_ESTOQUE, //
	...
	*/
	public ImportEstoqueOutput importEstoque(ImportEstoqueInput input, long timeout) {
		...
		ImportEstoqueOutput output = DtoJsonConverter.toDTO(resultMessage.getContent(), ImportEstoqueOutput.class);
		if (output == null) {
	...
	*/
	...
	public CompletableFuture<ImportEstoqueOutput> importEstoqueRequest(ImportEstoqueInput input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, ImportEstoqueOutput.class);
	}
	...
	* 
	*/
	public CompletableFuture<ImportEstoqueOutput> importEstoqueRequest(ImportEstoqueInput input, long timeout, TimeUnit unit) {
		MyServiceValidator.validate(input);
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, ImportEstoqueOutput.class, TimeUnit.MILLISECONDS.convert(timeout, unit));
	}
