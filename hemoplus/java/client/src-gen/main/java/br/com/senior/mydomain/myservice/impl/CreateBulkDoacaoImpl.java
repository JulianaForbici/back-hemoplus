...
import br.com.senior.mydomain.myservice.CreateBulkDoacaoInput;
import br.com.senior.mydomain.myservice.CreateBulkDoacaoOutput;
 ...
 * 
 */
public class CreateBulkDoacaoImpl {
	...
	public CreateBulkDoacaoImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(CreateBulkDoacaoInput input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				userId.getTenant(), //
				MyServiceConstants.DOMAIN, //
				MyServiceConstants.SERVICE, //
				MyServiceConstants.Commands.CREATE_BULK_DOACAO, //
	...
	*/
	public CreateBulkDoacaoOutput createBulkDoacao(CreateBulkDoacaoInput input, long timeout) {
		...
		CreateBulkDoacaoOutput output = DtoJsonConverter.toDTO(resultMessage.getContent(), CreateBulkDoacaoOutput.class);
		if (output == null) {
	...
	*/
	...
	public CompletableFuture<CreateBulkDoacaoOutput> createBulkDoacaoRequest(CreateBulkDoacaoInput input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, CreateBulkDoacaoOutput.class);
	}
	...
	* The 'createBulk' request primitive for the Doacao entity.
	*/
	public CompletableFuture<CreateBulkDoacaoOutput> createBulkDoacaoRequest(CreateBulkDoacaoInput input, long timeout, TimeUnit unit) {
		MyServiceValidator.validate(input);
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, CreateBulkDoacaoOutput.class, TimeUnit.MILLISECONDS.convert(timeout, unit));
	}
