...
import br.com.senior.mydomain.myservice.ListarEstoquePorHemocentroInput;
import br.com.senior.mydomain.myservice.ListarEstoquePorHemocentroOutput;
 ...
 * 
 */
public class ListarEstoquePorHemocentroImpl {
	...
	public ListarEstoquePorHemocentroImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(ListarEstoquePorHemocentroInput input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				userId.getTenant(), //
				MyServiceConstants.DOMAIN, //
				MyServiceConstants.SERVICE, //
				MyServiceConstants.Commands.LISTAR_ESTOQUE_POR_HEMOCENTRO, //
	...
	*/
	public ListarEstoquePorHemocentroOutput listarEstoquePorHemocentro(ListarEstoquePorHemocentroInput input, long timeout) {
		...
		ListarEstoquePorHemocentroOutput output = DtoJsonConverter.toDTO(resultMessage.getContent(), ListarEstoquePorHemocentroOutput.class);
		if (output == null) {
	...
	*/
	...
	public CompletableFuture<ListarEstoquePorHemocentroOutput> listarEstoquePorHemocentroRequest(ListarEstoquePorHemocentroInput input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, ListarEstoquePorHemocentroOutput.class);
	}
	...
	* queries de estoque
	*/
	public CompletableFuture<ListarEstoquePorHemocentroOutput> listarEstoquePorHemocentroRequest(ListarEstoquePorHemocentroInput input, long timeout, TimeUnit unit) {
		MyServiceValidator.validate(input);
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, ListarEstoquePorHemocentroOutput.class, TimeUnit.MILLISECONDS.convert(timeout, unit));
	}
