...
import br.com.senior.mydomain.myservice.ExportHemocentroInput;
import br.com.senior.mydomain.myservice.ExportHemocentroOutput;
 ...
 * 
 */
public class ExportHemocentroImpl {
	...
	public ExportHemocentroImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(ExportHemocentroInput input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				userId.getTenant(), //
				MyServiceConstants.DOMAIN, //
				MyServiceConstants.SERVICE, //
				MyServiceConstants.Commands.EXPORT_HEMOCENTRO, //
	...
	*/
	public ExportHemocentroOutput exportHemocentro(ExportHemocentroInput input, long timeout) {
		...
		ExportHemocentroOutput output = DtoJsonConverter.toDTO(resultMessage.getContent(), ExportHemocentroOutput.class);
		if (output == null) {
	...
	*/
	...
	public CompletableFuture<ExportHemocentroOutput> exportHemocentroRequest(ExportHemocentroInput input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, ExportHemocentroOutput.class);
	}
	...
	* 
	*/
	public CompletableFuture<ExportHemocentroOutput> exportHemocentroRequest(ExportHemocentroInput input, long timeout, TimeUnit unit) {
		MyServiceValidator.validate(input);
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, ExportHemocentroOutput.class, TimeUnit.MILLISECONDS.convert(timeout, unit));
	}
