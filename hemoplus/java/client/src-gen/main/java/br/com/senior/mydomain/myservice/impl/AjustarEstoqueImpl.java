...
import br.com.senior.mydomain.myservice.AjustarEstoqueInput;
import br.com.senior.mydomain.myservice.AjustarEstoqueOutput;
 ...
 * 
 */
public class AjustarEstoqueImpl {
	...
	public AjustarEstoqueImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(AjustarEstoqueInput input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				userId.getTenant(), //
				MyServiceConstants.DOMAIN, //
				MyServiceConstants.SERVICE, //
				MyServiceConstants.Commands.AJUSTAR_ESTOQUE, //
	...
	*/
	public AjustarEstoqueOutput ajustarEstoque(AjustarEstoqueInput input, long timeout) {
		...
		AjustarEstoqueOutput output = DtoJsonConverter.toDTO(resultMessage.getContent(), AjustarEstoqueOutput.class);
		if (output == null) {
	...
	*/
	...
	public CompletableFuture<AjustarEstoqueOutput> ajustarEstoqueRequest(AjustarEstoqueInput input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, AjustarEstoqueOutput.class);
	}
	...
	* actions de estoque
	*/
	public CompletableFuture<AjustarEstoqueOutput> ajustarEstoqueRequest(AjustarEstoqueInput input, long timeout, TimeUnit unit) {
		MyServiceValidator.validate(input);
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, AjustarEstoqueOutput.class, TimeUnit.MILLISECONDS.convert(timeout, unit));
	}
