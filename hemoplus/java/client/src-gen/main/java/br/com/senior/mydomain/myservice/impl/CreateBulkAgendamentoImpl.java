...
import br.com.senior.mydomain.myservice.CreateBulkAgendamentoInput;
import br.com.senior.mydomain.myservice.CreateBulkAgendamentoOutput;
 ...
 * 
 */
public class CreateBulkAgendamentoImpl {
	...
	public CreateBulkAgendamentoImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(CreateBulkAgendamentoInput input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				userId.getTenant(), //
				MyServiceConstants.DOMAIN, //
				MyServiceConstants.SERVICE, //
				MyServiceConstants.Commands.CREATE_BULK_AGENDAMENTO, //
	...
	*/
	public CreateBulkAgendamentoOutput createBulkAgendamento(CreateBulkAgendamentoInput input, long timeout) {
		...
		CreateBulkAgendamentoOutput output = DtoJsonConverter.toDTO(resultMessage.getContent(), CreateBulkAgendamentoOutput.class);
		if (output == null) {
	...
	*/
	...
	public CompletableFuture<CreateBulkAgendamentoOutput> createBulkAgendamentoRequest(CreateBulkAgendamentoInput input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, CreateBulkAgendamentoOutput.class);
	}
	...
	* The 'createBulk' request primitive for the Agendamento entity.
	*/
	public CompletableFuture<CreateBulkAgendamentoOutput> createBulkAgendamentoRequest(CreateBulkAgendamentoInput input, long timeout, TimeUnit unit) {
		MyServiceValidator.validate(input);
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, CreateBulkAgendamentoOutput.class, TimeUnit.MILLISECONDS.convert(timeout, unit));
	}
