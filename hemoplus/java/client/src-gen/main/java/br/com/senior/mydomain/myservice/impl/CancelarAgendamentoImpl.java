...
import br.com.senior.mydomain.myservice.CancelarAgendamentoInput;
import br.com.senior.mydomain.myservice.CancelarAgendamentoOutput;
...
public class CancelarAgendamentoImpl {
	...
	public CancelarAgendamentoImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(CancelarAgendamentoInput input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				userId.getTenant(), //
				MyServiceConstants.DOMAIN, //
				MyServiceConstants.SERVICE, //
				MyServiceConstants.Commands.CANCELAR_AGENDAMENTO, //
	...
	*/
	public CancelarAgendamentoOutput cancelarAgendamento(CancelarAgendamentoInput input, long timeout) {
		...
		CancelarAgendamentoOutput output = DtoJsonConverter.toDTO(resultMessage.getContent(), CancelarAgendamentoOutput.class);
		if (output == null) {
	...
	*/
	...
	public CompletableFuture<CancelarAgendamentoOutput> cancelarAgendamentoRequest(CancelarAgendamentoInput input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, CancelarAgendamentoOutput.class);
	}
	...
	* 
	*/
	public CompletableFuture<CancelarAgendamentoOutput> cancelarAgendamentoRequest(CancelarAgendamentoInput input, long timeout, TimeUnit unit) {
		MyServiceValidator.validate(input);
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, CancelarAgendamentoOutput.class, TimeUnit.MILLISECONDS.convert(timeout, unit));
	}
