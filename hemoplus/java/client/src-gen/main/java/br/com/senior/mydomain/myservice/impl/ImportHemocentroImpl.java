...
import br.com.senior.mydomain.myservice.ImportHemocentroInput;
import br.com.senior.mydomain.myservice.ImportHemocentroOutput;
 ...
 * 
 */
public class ImportHemocentroImpl {
	...
	public ImportHemocentroImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(ImportHemocentroInput input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				userId.getTenant(), //
				MyServiceConstants.DOMAIN, //
				MyServiceConstants.SERVICE, //
				MyServiceConstants.Commands.IMPORT_HEMOCENTRO, //
	...
	*/
	public ImportHemocentroOutput importHemocentro(ImportHemocentroInput input, long timeout) {
		...
		ImportHemocentroOutput output = DtoJsonConverter.toDTO(resultMessage.getContent(), ImportHemocentroOutput.class);
		if (output == null) {
	...
	*/
	...
	public CompletableFuture<ImportHemocentroOutput> importHemocentroRequest(ImportHemocentroInput input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, ImportHemocentroOutput.class);
	}
	...
	* 
	*/
	public CompletableFuture<ImportHemocentroOutput> importHemocentroRequest(ImportHemocentroInput input, long timeout, TimeUnit unit) {
		MyServiceValidator.validate(input);
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, ImportHemocentroOutput.class, TimeUnit.MILLISECONDS.convert(timeout, unit));
	}
