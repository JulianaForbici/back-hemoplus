 ...
 * 
 */
public class RetrieveUsuarioImpl {
	...
	public RetrieveUsuarioImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(Usuario.GetRequest input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				userId.getTenant(), //
				MyServiceConstants.DOMAIN, //
				MyServiceConstants.SERVICE, //
				MyServiceConstants.Commands.RETRIEVE_USUARIO, //
	...
	*/
	public Usuario retrieveUsuario(Usuario.GetRequest input, long timeout) {
		...
		Usuario output = DtoJsonConverter.toDTO(resultMessage.getContent(), Usuario.class);
		if (output == null) {
	...
	*/
	...
	public CompletableFuture<Usuario> retrieveUsuarioRequest(Usuario.GetRequest input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, Usuario.class);
	}
	...
	* The 'retrieve' request primitive for the Usuario entity.
	*/
	public CompletableFuture<Usuario> retrieveUsuarioRequest(Usuario.GetRequest input, long timeout, TimeUnit unit) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, Usuario.class, TimeUnit.MILLISECONDS.convert(timeout, unit));
	}
