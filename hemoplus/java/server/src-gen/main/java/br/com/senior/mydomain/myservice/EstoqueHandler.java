...
import static br.com.senior.mydomain.myservice.EstoqueEntity.SECURITY_RESOURCE;
...
import br.com.senior.mydomain.myservice.Estoque.PageRequest;
import br.com.senior.mydomain.myservice.Estoque.PagedResults;
import org.springframework.stereotype.Component;
import br.com.senior.platform.translationhub.api.TranslationHubApi;

@HandlerImpl(serviceClass = MyServiceConstants.class)
@Component("my_domain.my_service.EstoqueHandler")
public class EstoqueHandler implements CreateEstoque, CreateMergeEstoque, UpdateEstoque, DeleteEstoque, 
		 UpdateMergeEstoque, CreateBulkEstoque, RetrieveEstoque, ListEstoque, 
		 ImportEstoque, ExportEstoque {
	static final Logger logger = LoggerFactory.getLogger(EstoqueHandler.class);
	
	private static final String ID_FIELD_NAME = Arrays.stream(EstoqueEntity.class.getDeclaredFields())
	...
	@Inject
	EstoqueCrudService service;
	
	@Inject
	EstoqueDTOConverter dtoConverter;
	...
	public Estoque createEstoque(Estoque toCreate) {
		try {
			customValidator.validate(toCreate, entityDefProvider.getEntityDef("estoque"));
			EstoqueEntity entity = service.createEstoque(dtoConverter.toEntity(toCreate, true));
			...
			logger.error(e.getMessage(), e);
			throw new br.com.senior.messaging.model.ServiceException(br.com.senior.messaging.ErrorCategory.INTERNAL_ERROR, translationHubApi.getMessage("my_domain.my_service.estoque_illegalArgument", "An internal error occurred: "
				+ e.getClass().getSimpleName() + ". Please check the request parameters and try again."), e);
		} catch (Exception e) {
			logger.error(e.getMessage(), e);
			throw new br.com.senior.messaging.model.ServiceException(br.com.senior.messaging.ErrorCategory.INTERNAL_ERROR, translationHubApi.getMessage("my_domain.my_service.estoque_internalError", "An internal error occurred: "
				+ e.getClass().getSimpleName() + ". Please check the request parameters and try again."), e);
		}
	}
	...
	@Transactional
	@Override
	public Estoque createMergeEstoque(Estoque toCreate) {
		try {
			EstoqueEntity entity = service.createEstoque(dtoConverter.toEntity(toCreate, false));
			return dtoConverter.toDTO(entity);
	...
	public Estoque updateEstoque(Estoque toUpdate) {
		try {
			customValidator.validate(toUpdate, entityDefProvider.getEntityDef("estoque"));
			EstoqueEntity modified = doUpdate(toUpdate);
	...
	@Transactional
	@Override
	public Estoque updateMergeEstoque(Estoque toUpdate) {
		try {
			customValidator.validate(toUpdate, entityDefProvider.getEntityDef("estoque"));
			EstoqueEntity entity = service.retrieveEstoque(new Estoque.Id(toUpdate.id));
			dtoConverter.mergeEntity(entity, toUpdate);
			entity = service.updateEstoque(entity);
			return dtoConverter.toDTO(entity);
	...
	private EstoqueEntity doUpdate(Estoque toUpdate) {
		java.util.Optional<EstoqueEntity> optionalEntity = service.getRepository().findById(java.util.UUID.fromString(toUpdate.id));
		
		if(optionalEntity.isEmpty())
			throw new br.com.senior.messaging.model.ServiceException(br.com.senior.messaging.ErrorCategory.OBJECT_NOT_FOUND, translationHubApi.getExternalMessage("my_domain", "my_service", "my_domain.my_service.estoque_notFound", "Estoque not found with id " + toUpdate.id));
		
		EstoqueEntity entity = optionalEntity.get();
		
		dtoConverter.updateEntity(entity, toUpdate);
		return doUpdate(entity);
	}
	
	private EstoqueEntity doUpdate(EstoqueEntity entity) {
		return service.updateEstoque(entity);
	}
	...
	public void deleteEstoque(Id id) {
		try {
			...
			throw new ServiceException(ErrorCategory.CONFLICT, translationHubApi.getMessage("my_domain.my_service.estoque_dataIntegrityViolation", "Data integrity violation to Delete Estoque. Error: " + ex.getMessage()), ex);
		}
	}
	...
	@Override
	public Estoque retrieveEstoque(Estoque.GetRequest request) {
		EstoqueEntity entity = service.retrieveEstoque(new Estoque.Id(request.id));
	...
	@Transactional(readOnly = true)
	@Override
	public PagedResults listEstoque(PageRequest pageRequest) {
		...
		Page<EstoqueEntity> pages = this.service.listEstoquePageable(skip, top, orderBy, filter);
		List<EstoqueEntity> entities = pages.getContent();
		List<Estoque> dtos = null;
	...
	public CreateBulkEstoqueOutput createBulkEstoque(CreateBulkEstoqueInput toCreate) {
		if (toCreate != null) {
			List<Estoque> entitiesToCreate = toCreate.entities;
			if (entitiesToCreate != null && !entitiesToCreate.isEmpty()) {
				entitiesToCreate.forEach(bean -> {
					customValidator.validate(bean, entityDefProvider.getEntityDef("estoque"));
					bean.validate();
				});
				
				List<EstoqueEntity> entities = entitiesToCreate.stream().map(bean -> dtoConverter.toEntity(bean))
						.collect(Collectors.toList());
				this.service.createBulk(entities);
			}
		}
		return new CreateBulkEstoqueOutput();
	}
	...
	public ImportEstoqueOutput importEstoque(ImportEstoqueInput request) {
		...
		ImportEstoqueOutput output = new ImportEstoqueOutput(importer.getId());
		return output;
	}
	...
	@Transactional(readOnly = true)
	@Override
	public ExportEstoqueOutput exportEstoque(ExportEstoqueInput request) {
		/*
		 ...
		 */
		ExportEstoqueOutput output = new ExportEstoqueOutput(
				null /* exporter.getId() */);
		return output;
	}
	...
	class StartImportProcess extends Thread {
		...
		StartImportProcess(Importer importer, ImportConfig config) {
			...
			this.importer.addListener(new EstoqueImporterListener());
			this.config = config;
		...
		public void run() {
			...
			try {
				...
				try (FileReader reader = new FileReader(blob)) {
					...
					importer.setBeanClass(Estoque.class.getName());
					importer.setCrudService(EstoqueHandler.this.service, (config.batchSize > 0));
		...
		private File getHttpBlobFile(URI uri) {
			...
			try (CloseableHttpClient httpClient = HttpClients.createDefault();
				...
				File blob = File.createTempFile("estoque", "blob");
