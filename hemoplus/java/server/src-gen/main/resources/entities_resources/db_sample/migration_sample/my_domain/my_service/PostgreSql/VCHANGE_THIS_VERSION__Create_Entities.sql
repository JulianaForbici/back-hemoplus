/* Database: PostgreSql. Generation date: 2026-02-24 15:30:44:965 */
/* Entity Usuario */
...
alter table usuario_doacoes add constraint fk9gkyda0bfgexeefdxokqoj8jjdti foreign key (usuario_id) references usuario (id);
alter table usuario_doacoes add constraint fkfwer7wt9bd1kyewoqeduelmuiqbf foreign key (doacoes_id) references doacao (id);
alter table hemocentro_agendamentos add constraint fktdtknraucpapxfffpbtj61wnyscu foreign key (hemocentro_id) references hemocentro (id);
alter table hemocentro_agendamentos add constraint fktbwar7qwhqh90l187ui38nrynwub foreign key (agendamentos_id) references agendamento (id);
alter table hemocentro_doacoes add constraint fkkih57h3cwvgwx1xtmibiqfzn7nrb foreign key (hemocentro_id) references hemocentro (id);
alter table hemocentro_doacoes add constraint fklu4obhmuhm0ucnmubavz72vmvuhu foreign key (doacoes_id) references doacao (id);
alter table hemocentro_estoques add constraint fkemfxboqs5g8fwdvg8k9eb6ain9d0 foreign key (hemocentro_id) references hemocentro (id);
alter table hemocentro_estoques add constraint fklxwwxepz1h9oarlapvugb9upole9 foreign key (estoques_id) references estoque (id);
alter table agendamento add constraint fkuwysj1kw9zm6xiin6slxwzv12jhy foreign key (usuario) references usuario (id);
alter table agendamento add constraint fkfld22rbf9fegdh25skrlfivofcig foreign key (hemocentro) references hemocentro (id);
alter table doacao add constraint fkzdu5yb17oq8wnetuw4mply6z4yor foreign key (usuario) references usuario (id);
alter table doacao add constraint fkgrczzkhnozhhsbpnhzcwj6nva8yv foreign key (hemocentro) references hemocentro (id);
alter table doacao add constraint fkd30pxl5hgsf9guqtckjyhvduzqzo foreign key (agendamento) references agendamento (id);
alter table estoque add constraint fkxqugrgpmqguj8mynzlyylcy4dqi1 foreign key (hemocentro) references hemocentro (id);
